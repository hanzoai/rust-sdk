...
use hanzo_guard::{config::GuardConfig, types::SanitizeResult as GuardResult, Guard};

/// Sanitize extraction result using hanzo-guard
pub async fn sanitize_result(
    ...
    let sanitize_result = guard
        .sanitize_input(&result.text)
        .await
    ...
    match sanitize_result {
        ...
        GuardResult::Blocked { reason, .. } => Err(ExtractError::Blocked(reason)),
    }
}
...
pub async fn sanitize_text(
    text: &str,
    config: &ExtractorConfig,
) -> Result<(String, SanitizationInfo)> {
    let mut guard_config = GuardConfig::default();
    guard_config.pii.enabled = config.redact_pii;
    guard_config.injection.enabled = config.detect_injection;
    ...
    match sanitize_result {
        GuardResult::Clean(text) => Ok((text, SanitizationInfo::default())),
        GuardResult::Redacted { text, redactions } => {
            let pii_types: Vec<String> = redactions
                .iter()
                .map(|r| format!("{:?}", r.redaction_type))
...
pub async fn check_safety(text: &str, config: &ExtractorConfig) -> (bool, Option<String>) {
    ...
    match sanitize_result {
        ...
        Ok(GuardResult::Redacted { .. }) => (
            true,
            Some("Content contains PII that was redacted".to_string()),
        ),
...
mod tests {
    ...
    async fn test_sanitize_clean_text() {
        ...
        let (text, info) = sanitize_text("Hello world, this is a test.", &config)
            .await
            .unwrap();
    ...
    async fn test_sanitize_with_email() {
        let config = ExtractorConfig::default();
        let (text, info) = sanitize_text("Contact me at test@example.com for details.", &config)
            .await
            .unwrap();
