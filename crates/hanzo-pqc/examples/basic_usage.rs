...
async fn basic_kem_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        keypair.encap_key.key_bytes.len()
    );
    println!(
        "  Private key size: {} bytes",
        keypair.decap_key.key_bytes.len()
    );
    ...
    println!(
        "  Shared secret: {} bytes",
        encap_output.shared_secret.len()
    );

    // Receiver: Decapsulate to recover shared secret
    let recovered_secret = kem
        .decapsulate(&keypair.decap_key, &encap_output.ciphertext)
        .await?;
...
async fn hybrid_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (encap_key, decap_key) = hybrid_kem
        .generate_keypair(HybridMode::MlKem768X25519)
        .await?;
    println!("✓ Generated hybrid keypair");
    println!(
        "  PQ public key size: {} bytes",
        encap_key.pq_key.key_bytes.len()
    );
    println!(
        "  Classical public key size: {} bytes",
        encap_key.classical_key.key_bytes.len()
    );
    ...
    println!(
        ...
        ciphertext.classical_ciphertext.len()
    );
...
fn privacy_tier_example() {
    ...
    for tier in tiers {
        ...
        println!(
            "  Hybrid Mode: {}",
            if config.hybrid { "Enabled" } else { "Disabled" }
        );
        println!(
            "  FIPS Mode: {}",
            if config.fips_mode {
